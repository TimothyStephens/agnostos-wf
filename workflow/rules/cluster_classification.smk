rule cluster_classification:
    ...
    params:
        ...
        singl         = config["singl"],
        uniref_db     = config["ddir"] + "/" + config["uniref90_db"],
        uniref_prot   = config["ddir"] + "/" + config["uniref90_prot"],
        nr_db         = config["ddir"] + "/" + config["nr_db"],
        nr_prot       = config["ddir"] + "/" + config["nr_prot"],
        pfam          = config["ddir"] + "/" + config["pfam_shared_terms"],
        db_mode       = config["db_mode"],
        evalue_thr    = 0.6,
        hypo_thr      = 1.0,
        consensus     = config["wdir"] + "/scripts/consensus.sh",
        mmseqs_search = config["wdir"] + "/scripts/mmseqs_double_search.sh",
        evalue_filt   = config["wdir"] + "/scripts/evalue_filter.awk",
        ripgrep       = "rg",
        patterns      = config["wdir"] + "/scripts/hypothetical_grep.tsv",
        da_r          = config["wdir"] + "/scripts/cluster_pfam_domain_architectures.r",
        ...
        db_aln        = config["rdir"] + "/cluster_classification/ref_noannot_clu_aln",
        db_cons       = config["rdir"] + "/cluster_classification/ref_noannot_clu_cons",
        ref_cons      = config["rdir"] + "/cluster_classification/refined_not_annotated_cluster_cons.fasta",
        uniref_res    = config["rdir"] + "/cluster_classification/noannot_vs_uniref90.tsv",
        nr_res        = config["rdir"] + "/cluster_classification/uniref-nohits_vs_NR.tsv",
        dom_arch      = config["rdir"] + "/cluster_classification/cluster_pfam_domain_architectures.tsv",
    ...
    log:
        config["rdir"] + "/logs/class_stdout.log",
    benchmark:
        config["rdir"] + "/benchmarks/clu_class.tsv"
    shell:
        """
        (
        set -e
        ...
        if [ ! -f {params.ref_cons} ]; then
            ...
            if [ {params.singl} == "true" ]; then
                join -11 -21 -v1 \
                    <(awk '{{print $2}}' {params.s_all} | sort -k1,1) \
                    <(awk '{{print $1}}' {params.s_annot} | sort -k1,1) \
                  > {params.outdir}/singletons_not_annot.txt
                ...
                {params.filterbyname} in={params.outdir}/genes.fasta \
                                      ...
                                      include=t ignorejunk overwrite="true"
        ...
        if [ ! -f {params.outdir}/noannot_vs_uniref90_hits.txt ]; then
            ...
            {params.filterbyname} in={params.ref_cons} \
                                  ...
                                  include=f ignorejunk overwrite="true"
        ...
        if [ {params.singl} == "true" ]; then
            ...
            # cluster EU
            join -11 -21 -v1 \
                <(sort -k1,1 {params.outdir}/all_eu) \
                <(sort -k1,1 {params.outdir}/singletons_not_annot.txt) \
              > {output.eu}
            # singleton EU
            join -11 -21 \
                <(sort -k1,1 {params.outdir}/all_eu) \
                <(sort -k1,1 {params.outdir}/singletons_not_annot.txt) \
              > {params.outdir}/singl_eu
        else
        ...
        join -11 -21 -v1 \
            <(sort {params.outdir}/noannot_vs_uniref90_hits.txt) \
            <(sort {params.outdir}/noannot_vs_uniref90_hypo.txt) \
          > {params.outdir}/noannot_vs_uniref90_char.txt
        # Not-hypothetical hits from the search against NCBI nr
        join -11 -21 -v1 \
            <(sort {params.outdir}/uniref-nohits_vs_NR_hits.txt) \
            <(sort {params.outdir}/uniref-nohits_vs_NR_hypo.txt) \
          > {params.outdir}/uniref-nohits_vs_NR_char.txt
        
        if [ {params.singl} == "true" ]; then
            cat {params.outdir}/noannot_vs_uniref90_char.txt \
                {params.outdir}/uniref-nohits_vs_NR_char.txt \
              > {params.outdir}/all_kwp
            # cluster kWP
            join -11 -21 -v1 \
                <(sort -k1,1 {params.outdir}/all_kwp) \
                <(sort -k1,1 {params.outdir}/singletons_not_annot.txt) \
              > {output.kwp}
            # singleton KWP
            join -11 -21 \
        ...
        cat {params.outdir}/noannot_vs_uniref90_E{params.evalue_thr}_prot.tsv \
            {params.outdir}/uniref-nohits_vs_NR_E{params.evalue_thr}_prot.tsv \
          > {params.outdir}/noannot_uniref_nr_annotations.tsv
        ## KWP annotations
        join -11 -21 \
            <(sort -k1,1 {output.kwp} ) \
            <(sort -k1,1 {params.outdir}/noannot_uniref_nr_annotations.tsv) \
        ...
        if [ {params.singl} == "true" ]; then
             cat <(awk 'NR>1 && $9=="DUF"{{print $1}}' {params.dom_arch}) \
                 ...
                 {params.outdir}/uniref-nohits_vs_NR_hypo.txt \
               > {params.outdir}/all_gu
             # cluster GU
             join -11 -21 -v1 \
                 <(sort -k1,1 {params.outdir}/all_gu) \
                 <(awk '{{print $2}}' {params.s_all} | sort -k1,1 ) \
               > {output.gu}
             # singleton GU
             join -11 -21 \
        ...
        awk 'NR>1 && $9=="DUF"{{print $1,"PFAM","0.0",$5}}' {params.dom_arch} \
          >> {params.outdir}/gu_annotations.tsv
        ...
        if [ {params.singl} == "true" ]; then
             ...
             cat <(awk '{{print $1,"EU"}}' {params.outdir}/singl_eu) \
                 ...
                 <(awk '{{print $1,"K"}}' {params.outdir}/singl_k) \
              > {params.outdir}/singl_gene_categ
            # singletons gene cluster categories
            join -12 -21 \
                <(sort -k2,2 {params.s_all}) \
        ...
        ## K annotations
        awk -vOFS='\\t' 'NR>1 && $9!="DUF"{{print $1,"PFAM","0.0",$5}}' {params.dom_arch} \
