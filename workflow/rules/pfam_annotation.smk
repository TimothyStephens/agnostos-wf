rule pfam_annotation:
    ...
    benchmark:
        config["rdir"] + "/benchmarks/pfam_annot.tsv"
    shell:
        """
        (
        set -x
        ...
        if [ ! -s {params.annot} ]; then
            ...
            {params.mpi_runner} {params.hmmer_bin} --mpi --cut_ga -Z "${{N}}" --domtblout {params.hmmout} -o {params.hmmlog} {params.pfamdb} {input.gp}
        ...
        # Parse the results
        {params.parser} {params.annot} {params.evalue} {params.coverage} > {output.pf_annot}
