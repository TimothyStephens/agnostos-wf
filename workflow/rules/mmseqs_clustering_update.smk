rule mmseqs_clustering_update:
    ...
    log:
        config["rdir"] + "/logs/mmseqs_clustering_update.log",
    benchmark:
        config["rdir"] + "/benchmarks/mmseqs_clustering_update.tsv"
    shell:
        """
        (
        set -x
        ...
        {params.mmseqs_bin} createdb {input.new_seqs} {params.new_seqdb}
...
rule mmseqs_cluster_update_done:
    ...
    output:
        updt_done = touch(config["rdir"] + "/mmseqs_clustering/updt.done")
    run:
