rule cluster_refinement:
      ...
      params:
        ...
        toremove_sh   = config["wdir"] + "/scripts/remove_orfs.sh",
      ...
      log:
        config["rdir"] + "/logs/cluster_refinement.log",
      benchmark:
        config["rdir"] + "/benchmarks/cluster_refinement.tsv"
      shell:
        """
        (
        set -x
        ...
        if [[ ! -s {params.toremove} ]]; then
            ...
            join -11 -21 -v2 \
                <(awk '$5>=0.3 && !seen[$2]++{{print $2}}' {input.sp_sh} | sort -k1,1) \
                <(grep -v 'cl_name' {input.good} | awk '{{print $1}}' | sort -k1,1) \
              > {params.good_noshadow}
            ...
            join -11 -21 \
                <(sort -k1,1 {input.cval_rej}) \
                <(sort -k1,1 {params.good_noshadow}) \
              > {params.rej}
        ...
        if [[ -s {params.toremove} ]]; then
            ...
            if [[ ! -s {params.refdb} ]]; then
                {params.mpi_runner} {params.mmseqs_bin} apply {params.tmpdb} {params.refdb} --threads {threads} \
                   -- {params.toremove_sh} {params.toremove_cl}
            fi
            ...
            join -12 -21 -v1 \
                <(sort -k2,2 --parallel={threads} -T {params.local_tmp} {params.tmp1}) \
                <(sort -k1,1 --parallel={threads} -T {params.local_tmp} {params.toremove}) \
              > {params.tmp} && mv {params.tmp} {params.tmp1}
        else
        ...
        if [[ -s {params.new_noannot} ]]; then
          ...
          join -11 -21 -v1 \
              <(awk '!seen[$1,$2,$3]++' {output.ref_annot} | sort -k1,1 --parallel={threads} -T {params.local_tmp}) \
              <(sort {params.new_noannot}) \
            > {params.tmp}
          
          join -11 -21 \
              <(awk '!seen[$1,$2,$3]++{{print $1,$2}}' {output.ref_annot} | sort -k1,1 --parallel={threads} -T {params.local_tmp}) \
              <(sort {params.new_noannot}) \
            > {output.ref_noannot}
        ...
        join -11 -21 \
            <(sort -k1,1 --parallel={threads} -T {params.local_tmp} ${{PARTIAL}}) \
            <(sort -k1,1 --parallel={threads} -T {params.local_tmp} {params.tmp1}) \
          > {params.tmp2}
        ...
        join -11 -21 \
              <(sort -k1,1 --parallel={threads} -T {params.local_tmp} {params.tmp2}) \
              <(awk '{{print $3,$2,$4,$5}}' {params.clu_info} \
            | sort -k1,1 --parallel={threads} -T {params.local_tmp}) \
            | sort -k3,3 --parallel={threads} -T {params.local_tmp} \
            | awk -vOFS='\\t' '{{print $3,$1,$2,$6,$5}}' \
            > {params.tmp3}
        
        # Add new representatives from validation
        # Columns tmp5: 1:cl_name|2:orf|3:partial|4:cl_size|5:orf_len|6:new_repr
        join -11 -21 \
            <(sort -k1,1 --parallel={threads} -T {params.local_tmp} {params.tmp3}) \
            <(awk '{{print $1,$2}}' {input.val_res} | sort -k1,1 --parallel={threads} -T {params.local_tmp}) \
          > {params.tmp4}
